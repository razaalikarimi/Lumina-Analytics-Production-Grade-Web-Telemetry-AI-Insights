...
import { detectTrafficAnomalies, TrafficDataPoint } from '@/lib/gemini';
...
export async function GET(request: Request) {
  try {
    ...
    // 2. Call Gemini
    const anomalies = await detectTrafficAnomalies(historicalData);

    return NextResponse.json({ anomalies });
  } catch (error) {
    console.error('API AI Anomalies GET error:', error);
